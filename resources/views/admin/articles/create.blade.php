...
@section('content')
    ...
    <form action="{{ route('article.store') }}" method="POST" id="articleForm">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="meta_key" id="meta_key" class="form-control" placeholder="pisahkan dengan koma" value="{{ old('meta_key') }}">
        </div>
        <div class="mb-3">
            <label class="form-label">JSON-LD Schema</label>
            <textarea name="schema" id="schema" class="form-control" placeholder="Masukkan JSON-LD Schema (otomatis diisi jika menggunakan AI)">{{ old('schema') }}</textarea>
            @error('schema')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
